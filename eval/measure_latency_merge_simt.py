        ...
        for mkey in ["self_attn", "mlp"]:
            ...
            if mkey == "mlp" and args.use_inc_mlp:
                mlp = IncoherentMLP.gen_layer_from_quantizer_str_and_key(
                                                    model.config, 
                                                    quant_dir, 
                                                    quantizer_str_up=qdict[f"{i}_mlp.up_proj"][0], 
                                                    quantizer_str_gate=qdict[f"{i}_mlp.gate_proj"][0], 
                                                    quantizer_str_down=qdict[f"{i}_mlp.down_proj"][0], 
                                                    ...
                                                    dummy=args.dummy,
                                                    use_simt=True,
                                                    use_simt_u=qdict[f"{i}_mlp.up_proj"][1] == "1",
                                                    use_simt_g=qdict[f"{i}_mlp.gate_proj"][1] == "1",
                                                    use_simt_d=qdict[f"{i}_mlp.down_proj"][1] == "1",
                                                )
                ...
                attn = IncoherentSdpaAttention.gen_layer_from_quantizer_str_and_key(
                                                    ...
                                                    i,
                                                    quant_dir, 
                                                    quantizer_str_q=qdict[f"{i}_self_attn.q_proj"][0], 
                                                    quantizer_str_k=qdict[f"{i}_self_attn.k_proj"][0], 
                                                    quantizer_str_v=qdict[f"{i}_self_attn.v_proj"][0], 
                                                    quantizer_str_o=qdict[f"{i}_self_attn.o_proj"][0], 
                                                    ...
                                                    dummy=args.dummy,
                                                    use_simt=True,
                                                    use_simt_q=qdict[f"{i}_self_attn.q_proj"][1] == "1",
                                                    use_simt_k=qdict[f"{i}_self_attn.k_proj"][1] == "1",
                                                    use_simt_v=qdict[f"{i}_self_attn.v_proj"][1] == "1",
                                                    use_simt_o=qdict[f"{i}_self_attn.o_proj"][1] == "1",
                                                )
                ...
                for skey in SKEY_DICT[mkey]:
                    ...
                    setattr(getattr(layer, mkey), skey, IncoherentLinear.gen_layer_from_quantizer_str_and_key(
                        model.config, quant_dir, qdict[f"{i}_{key}"][0], f"{i}_{key}", merge_layers=True, use_simt=qdict[f"{i}_{key}"][1]=="1", dummy=args.dummy))
...
def main(hf_path, compile, max_tokens, enable_flash, num_samples, args, print_result=False):
    ...
    save_dir = args.qdict_path.replace('.pt', f'_latency_bs{args.batch_size}.pt')
    os.makedirs(os.path.dirname(save_dir), exist_ok=True)
    torch.save(result_dict, save_dir)

    # save text
    save_dir = args.qdict_path.replace('.pt', f'_latency_bs{args.batch_size}.txt')
    os.makedirs(os.path.dirname(save_dir), exist_ok=True)
    str_ = ""
