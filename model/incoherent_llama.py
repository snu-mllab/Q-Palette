...
class LlamaMLP(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        self.w1w3 = IncoherentLinear(config.hidden_size, config.intermediate_size*2, bias=False, dtype=config.torch_dtype)
        self.w2 = IncoherentLinear(config.intermediate_size, config.hidden_size, bias=False, dtype=config.torch_dtype)
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.wqkv = IncoherentLinear(config.hidden_size, total_head_dim, bias=False, dtype=config.torch_dtype)
        self.wo = IncoherentLinear(config.hidden_size, config.hidden_size, bias=False, dtype=config.torch_dtype)
...
class IncoherentLlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
