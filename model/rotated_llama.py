...
class LlamaMLP(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        self.SU4 = nn.Parameter(torch.ones(config.intermediate_size))
        self.hadK4, self.K4 = get_hadK(config.intermediate_size)
        self.hadK4 = torch.nn.Parameter(self.hadK4)
        self.w1w3 = nn.Linear(config.hidden_size, config.intermediate_size*2, bias=False)
    ...
    def forward(self, x):
        if self.config.pretraining_tp > 1:
            ...
            tmp = (self.act_fn(w1_out) * w3_out)
            tmp = matmul_hadU_cuda(tmp * self.SU4, self.hadK4, self.K4) 
            down_proj = self.w2(tmp)
        return down_proj
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        total_head_dim = (config.num_attention_heads + 2 * config.num_key_value_heads) * config.head_dim
        self.kv_size = self.num_key_value_heads * config.head_dim
        self.wqkv = nn.Linear(config.hidden_size, total_head_dim, bias=False)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config.pretraining_tp > 1:
            ...
            query_states, key_states, value_states = self.wqkv(hidden_states).split([self.dim, self.kv_size, self.kv_size], dim=-1)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.SU1 = torch.nn.Parameter(torch.ones(config.hidden_size))
        self.hadK1, self.K1 = get_hadK(config.hidden_size)
        self.hadK1 = torch.nn.Parameter(self.hadK1)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            inputs_embeds = matmul_hadU_cuda(inputs_embeds * self.SU1, self.hadK1, self.K1)
...
class RotatedLlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.lm_head = nn.Linear(config.hidden_size,
                                 config.vocab_size,
                                 bias=False)
        self.SU1 = nn.Parameter(torch.ones(config.hidden_size))  
        self.head_dim = getattr(config, "head_dim",
                                config.hidden_size // config.num_attention_heads)
        self.SU2 = nn.Parameter(torch.ones(self.head_dim))
        self.SU4 = nn.Parameter(torch.ones(config.intermediate_size))
        self.hadK1, self.K1 = get_hadK(config.hidden_size)
        self.hadK1 = torch.nn.Parameter(self.hadK1)
        # Initialize weights and apply final processing
    ...
    def forward(
        ...
        hidden_states = matmul_hadUt_cuda(hidden_states, self.hadK1, self.K1) * self.SU1.to(hidden_states.device)
...
transformer_configs = {
    "Llama-2-7b-hf": dict(model_name="Llama-2-7b-hf", block_size=4096, n_layer=32, n_head=32, n_local_heads=32, dim=4096, intermediate_size=11008, vocab_size=32000, rope_base=10000),
